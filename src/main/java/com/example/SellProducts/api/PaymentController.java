...
public class PaymentController {
    ...
    @GetMapping
    public ResponseEntity<List<PaymentDTO2>> getAllPayments() {
        return ResponseEntity.ok().body(paymentService.getAllPayments());
    }

    @GetMapping("/{id}")
    public ResponseEntity<PaymentDTO2> getPaymentById(@PathVariable("id") Long id) {
        return ResponseEntity.ok().body(paymentService.getPaymentById(id));
    }

    @GetMapping("/order/{orderId}")
    public ResponseEntity<List<PaymentDTO2>> getPaymentsByOrderId(@PathVariable("orderId") Long orderId, @RequestParam methodPayment method) {
        return ResponseEntity.ok().body(paymentService.getPaymentsByOrderId(orderId, method));
    }

    @GetMapping("/date-range")
    public ResponseEntity<List<PaymentDTO2>> getPaymentsForDateRange(@RequestParam LocalDate start, @RequestParam LocalDate end) {
        return ResponseEntity.ok().body(paymentService.getPaymentsForDateRange(start, end));
    }

    @PostMapping
    public ResponseEntity<PaymentDTO2> createPayment(@Valid @RequestBody CreatePaymentDTO createPaymentDTO) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<PaymentDTO2> UpdatePayment(@PathVariable("id") Long id, @RequestBody UpdatePaymentDTO updatePaymentDTO) {
